...
export function AdminSidebar({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-screen w-64 shrink-0 flex-col border-r border-border bg-background transition-transform duration-200 dark:bg-slate-950 lg:static lg:z-auto lg:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full",
        )}
      >
        {/* Brand */}
        <div className="flex h-14 items-center gap-1.5 border-b border-border px-4">
          <Link href="/admin" aria-label="BatchKart Admin" onClick={onClose}>
            <Logo />
          </Link>
          ...
          <button
            ...
            className="ml-auto grid size-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden"
          >
            <X className="size-5" />
          </button>
        </div>
